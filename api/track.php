...
try {
    ...
    $type=substr((string)($in['event']??'page_view'),0,50);$path=substr((string)($in['path']??''),0,500);$referrer=substr((string)($in['referrer']??''),0,1000);
    ...
    $metadata=$in['metadata']??[];if(!is_array($metadata))$metadata=[];$metadataJson=json_encode($metadata,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);$now=gw_now();$db=gw_db();$db->beginTransaction();
